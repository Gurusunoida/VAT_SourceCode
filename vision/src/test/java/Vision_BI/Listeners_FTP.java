...
public class Listeners_FTP implements ITestListener {

	private static final String REPORTS_PATH = System.getProperty("user.dir") + File.separator + "Reports"
			+ File.separator + "VisionBI(AdvFTP)_21-May-2025.html";
	...
	private static final String SCREENSHOT_PATH = System.getProperty("user.dir") + File.separator + "test-output"
			+ File.separator + "Screenshot" + File.separator + "FTP" + File.separator;
	...
	public void onStart(ITestContext context) {
		...
		System.out.println("Execution of Adv_FTP_Application started");
		reporter = new ExtentSparkReporter(REPORTS_PATH);

		try {
			String customFTPcss = readCSSFile("src/main/resources/static/assets/customFTP.css");
			reporter.config().setCss(customFTPcss);
		} catch (IOException e) {
			e.printStackTrace();
		}

		reporter.config().setDocumentTitle("VAT_Report_AdvFTP");
		reporter.config().setReportName("AdvFTP_Report");
		extent = new ExtentReports();
		extent.attachReporter(reporter);
		extent.setSystemInfo("Date", "08-Apr-2025");
		...
		extent.setSystemInfo("Product", "VisionBI");
		extent.setSystemInfo("Info", "AdvFTP-VAT_Report");
	}
	...
	public void onFinish(ITestContext context) {
		System.out.println("Execution of AdvFTP_Application Completed");
			...
			if (!classTotal.get(className)) {
				...
				if (classTest != null) {
					String countDetails = String.format(
							"<pre>Total Testcases           : %d\n" + "Passed Testcases          : %d\n"
									+ "Failed Testcases          : %d\n" + "Ignored/Skipped Testcases : %d</pre>",
	...
	private void attachScreenshotIfAvailable(ITestResult result) throws IOException {
		...
		if (testInstance instanceof BaseClassFTP) {
			driver = ((BaseClassFTP) testInstance).getDriver();
		}
